...
def plot_histogram(data, column_name, file_name):
    ...
    save_plot(plt, f'/Users/berkesavasci/Documents/Datascience/World_Happiness_Graphs/graphs_LadderScore/histogram_{column_name.replace(" ", "_")}_{os.path.basename(file_name)}.png')

def normality_testing(data, column_name):
    stats.probplot(data, plot=plt)
    plt.title(f"QQ Diagramm - {column_name} Data")
    save_plot(plt, f'/Users/berkesavasci/Documents/Datascience/World_Happiness_Graphs/graphs_LadderScore/QQ_plot_{column_name.replace(" ", "_")}_{os.path.basename(file_name)}.png')
...
def create_and_save_scatterplot(file_name):
    ...
    df['Logged GDP per capita'] = pd.to_numeric(df['Logged GDP per capita'].replace({',': '.'}, regex=True), errors='coerce')
    ...
    save_plot(plt, f'/Users/berkesavasci/Documents/Datascience/World_Happiness_Graphs/graphs_LadderScore/correlation_{os.path.basename(file_name)}.png')
...
def compute_correlation(file_name):
    ...
    df = df.rename(columns={'Ladder score': 'Ladder Score', 'Logged GDP per capita': 'GDP per Capita', 'Social support': 'Social Support', 'Healthy life expectancy': 'Life expectancy', 'Freedom to make life choices': 'Freedom of Choice', 'Generosity': 'Generosity', 'Perceptions of corruption': 'Corruption'})
    new_columns = ['Ladder Score', 'GDP per Capita', 'Social Support', 'Life expectancy', 'Freedom of Choice', 'Generosity', 'Corruption']
    ...
    save_plot(plt, f'/Users/berkesavasci/Documents/Datascience/World_Happiness_Graphs/graphs_LadderScore/correlationTable{os.path.basename(file_name)}.png')
...
file_list = ['Europe.xlsx']

for file_name in file_list:
    compute_correlation(file_name)
    create_and_save_histograms(file_name, ['Ladder score', 'Logged GDP per capita', 'Social support', 'Perceptions of corruption', 'Freedom to make life choices'])
